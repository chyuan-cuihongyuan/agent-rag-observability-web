...
- **评测工作台**（工单 0140）：三池与版本管理（golden/challenge/wrong、复制新版本/冻结）、Case 挖掘候选池（来源筛选/上下文快照/批量回填错题集/归因标注）、归因四分层分布视图、门禁 PASS/BLOCK 结果与历史、巡检拨测状态卡（工单 0137-0139 后端配套）、调度健康表（工单 0189：定时任务最近运行/结果/下次提示，后端未就绪时空数据兜底）
...
| `/eval/workbench` | 评测工作台（三池/Case 池/归因/门禁/巡检/调度健康六区块） |
