...
export default function TracesPage() {
  ...
  // AUTOLOOP al-42 / 工单 1042：traceList 查询键化（第三批）。
  // placeholderData 保上一页数据：翻页不闪白（v5 官方翻页模式）。
  const tracesQuery = useQuery({
    queryKey: ["traces", page, sessionId, userId, agentId, branchType, agentStatus],
    queryFn: () =>
      queryApi.traceList({
  ...
  const traces: TraceListItem[] = tracesQuery.data?.list ?? [];
  const total = tracesQuery.data?.total ?? 0;
  const loading = tracesQuery.isPending;
  const error = tracesQuery.isError ? "Trace 列表加载失败，请检查服务状态后重试" : null;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {error ? (
            <ErrorState message={error} onRetry={() => tracesQuery.refetch()} />
          ) : loading ? (
            <LoadingState text="正在加载 Trace 列表..." />
          ) : traces.length === 0 ? (
            <EmptyState text="暂无数据" description="调整筛选条件或稍后重试" />
          ) : (
...
function StatusBadge({ status }: { status: string }) {
  const variant =
    status === "SUCCESS" ? "default" : status === "FAIL" ? "destructive" : "secondary";
...
function BranchBadge({ branch }: { branch?: string }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded text-xs ${colorMap[branch] || "bg-gray-100 text-gray-800"}`}
    >
      {branch}
    </span>
  );
